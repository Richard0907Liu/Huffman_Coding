...
void Min_heap()
{
	...
	for (int j = 0; j < 2; j++)  // make sure sort correctly
	{
...
void write_bit(FILE *input, FILE *outputFile)
{
	char temp = 0;
	char ch;
	...
	while ((fread(&ch, sizeof(char), 1, input)) != 0)
	{
		search_node(LL_for_encoding, ch, outputFile);
	}
	
	byte = byte | temp; // write last byte;
	fwrite(&byte, sizeof(char), 1, outputFile);

	rewind(outputFile);
	fwrite(&padding, sizeof(int), 1, outputFile); // write padding at first
}

void search_node(node *ptr, char ch, FILE *output)  
{  // ### using LL_for_encoding to search ###
	if (ptr != NULL)
	{
		if (ch == ptr->ch) // write bits to an output file
		{
			bit_writing(ptr->node_code, output);
			return;
		}
		search_node(ptr->next, ch, output);
	}
}
...
{  
	...
	for (int i = 0; i < strlen(ch); i++)
	{
		...
		count++;
		if (count == 8)  // write byte into file 
		{
...
int main(int argc, char **argv) 
{
	...
	if ((strcmp(argv[1], " ") != 0))
	{
		...
		encode(argv[1], (strcat(filename, ".huf")));
		if (remove(argv[1]) == 0)
